...
INSERT INTO `character_classes` (`character_id`, `class_id`, `subclass_id`, `level`, `is_primary`) VALUES
(@shisui_id, (SELECT `id` FROM `rule_classes` WHERE `code` = 'monaco' LIMIT 1), (SELECT `id` FROM `rule_subclasses` WHERE `code` = 'monaco_guerriero_degli_elementi' LIMIT 1), 4, 1)
ON DUPLICATE KEY UPDATE `subclass_id` = VALUES(`subclass_id`), `level` = VALUES(`level`), `is_primary` = VALUES(`is_primary`);
INSERT INTO `character_ability_bases` (`character_id`, `ability_id`, `base_value`, `method`, `notes`) VALUES
...
(@shisui_id, (SELECT `id` FROM `rule_abilities` WHERE `code` = 'cha' LIMIT 1), 14, 'imported', 'Import da shisui.json')
ON DUPLICATE KEY UPDATE `base_value` = VALUES(`base_value`), `method` = VALUES(`method`), `notes` = VALUES(`notes`);
INSERT INTO `character_proficiencies` (`character_id`, `proficiency_type`, `proficiency_ref`, `rank`, `origin_type`, `notes`)
...
INSERT INTO `character_modifiers` (`character_id`, `target_code`, `target_ref`, `operation`, `value_number`, `origin_type`, `origin_ref`, `duration_type`, `notes`)
SELECT @shisui_id, 'max_hp', NULL, 'set', 31, 'manual', 'import_shisui_max_hp_base', 'permanent', 'Valore importato dallo stato corrente del JSON demo.'
...
INSERT INTO `character_resources` (`character_id`, `code`, `name`, `max_value_snapshot`, `current_value`, `recovery`, `origin_type`, `notes`)
SELECT @shisui_id, 'ki', 'Ki', 4, 4, 'short_rest', 'class', 'Import da JSON demo.'
...
INSERT INTO `character_resources` (`character_id`, `code`, `name`, `max_value_snapshot`, `current_value`, `recovery`, `origin_type`, `notes`)
SELECT @shisui_id, 'dark_touched_invisibility', 'Contaminazione Oscura: Invisibilita', 1, 1, 'long_rest', 'feat', 'Talento Contaminazione oscura, Manuale del Giocatore 2024 pag. 204.'
WHERE NOT EXISTS (SELECT 1 FROM `character_resources` WHERE `character_id` = @shisui_id AND `code` = 'dark_touched_invisibility');
INSERT INTO `character_resources` (`character_id`, `code`, `name`, `max_value_snapshot`, `current_value`, `recovery`, `origin_type`, `notes`)
SELECT @shisui_id, 'dark_touched_color_spray', 'Contaminazione Oscura: Spruzzo Colorato', 1, 1, 'long_rest', 'feat', 'Talento Contaminazione oscura, Manuale del Giocatore 2024 pag. 204.'
WHERE NOT EXISTS (SELECT 1 FROM `character_resources` WHERE `character_id` = @shisui_id AND `code` = 'dark_touched_color_spray');
INSERT INTO `character_spellcasting_profiles` (`character_id`, `class_id`, `source_type`, `casting_ability_id`, `preparation_mode`, `spell_list_ref`, `prepared_count_formula`, `focus_rules_json`)
SELECT @shisui_id, (SELECT `id` FROM `rule_classes` WHERE `code` = 'monaco' LIMIT 1), 'class', (SELECT `id` FROM `rule_abilities` WHERE `code` = 'cha' LIMIT 1), 'prepared', 'monaco', '4', '{"imported_from":"shisui.json"}'
...
AND NOT EXISTS (SELECT 1 FROM `character_spells` WHERE `character_id` = @shisui_id AND `spell_id` = `rule_spells`.`id`);
INSERT INTO `character_spells` (`character_id`, `spell_id`, `origin_type`, `is_known`, `is_prepared`, `prepared_context`)
SELECT @shisui_id, `id`, 'feat', 1, 1, 'contaminazione_oscura'
FROM `rule_spells` WHERE `code` = 'invisibilita'
AND NOT EXISTS (SELECT 1 FROM `character_spells` WHERE `character_id` = @shisui_id AND `spell_id` = `rule_spells`.`id`);
INSERT INTO `character_spells` (`character_id`, `spell_id`, `origin_type`, `is_known`, `is_prepared`, `prepared_context`)
SELECT @shisui_id, `id`, 'feat', 1, 1, 'contaminazione_oscura'
FROM `rule_spells` WHERE `code` = 'spruzzo-colorato'
AND NOT EXISTS (SELECT 1 FROM `character_spells` WHERE `character_id` = @shisui_id AND `spell_id` = `rule_spells`.`id`);
INSERT INTO `character_inventory` (`character_id`, `equipment_id`, `custom_name`, `quantity`, `origin_type`, `notes`)
